...
function PluginManager:InitPluginsNotInstalled()
    ...
    -- [Community fix] GetNumAddOns/GetAddOnInfo were moved to C_AddOns in WoW 11.x
    -- and the old globals are nil on TWW builds. Resolve defensively.
    local getNum  = (C_AddOns and C_AddOns.GetNumAddOns)  or GetNumAddOns
    local getInfo = (C_AddOns and C_AddOns.GetAddOnInfo)  or GetAddOnInfo
    local addons = {} do
        if getNum and getInfo then
            for i = 1, getNum() do
                addons[getInfo(i)] = true
            end
        end
    end
...
function PluginManager:EnableModuleWithAddonLoaded(module, addon)
    ...
    -- [Community fix] IsAddOnLoaded moved to C_AddOns in WoW 11.x
    local isLoaded = (C_AddOns and C_AddOns.IsAddOnLoaded) or IsAddOnLoaded
    if isLoaded and not isLoaded(addon) then
